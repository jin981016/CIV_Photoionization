...
!!!
file = pd.read_csv('/home/jin/CIV_Photoionization/CIV_emissivity.txt',sep='\t',header=0)  # emissivity file 읽기
radius,emiss= interpolation(file['Radius_kpc'].to_numpy(), file['Emissivity'].to_numpy()) # 1행 이름 , 1열 radius, 2열 ,emissivity file 읽기
radius_max = 100  
...
n , loop = 0 , 0 
    ...
    if (r2 <= px/p_max):
        ...
        x_i = r0 * np.sin(phi) * np.cos(theta) *kpc
        y_i = r0 * np.sin(phi) * np.sin(theta) * kpc
        z_i = r0 * np.cos(phi) * kpc
!!!!!
