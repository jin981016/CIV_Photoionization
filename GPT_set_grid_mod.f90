...
double precision, allocatable :: radius_r(:), density_r(:)
integer :: n_points
...
! Read data from file
call read_data_file('/home/jin/CIV_Photoionization/CIV_number_density.txt', radius_r, density_r, n_points)
...
if(mpar%h_rank .eq. master) then
	...
	##
	grid%den(ix,iy,iz) = find_y(temp1, radius_r, density_r, n_points)  ! Interpolation-based density assignment
	##
